...
function viewBookmarks(bookmarks){
    ...
    if(bookmarks.length === 0){
        bookmarkSection.innerHTML = `<i>No Bookmarks Available</i>`;
        return;
    }

    bookmarks.forEach(bookmark => {
        return addNewBookmark(bookmark);
    });
}
...
function addNewBookmark(bookmark){
    ...
    bookmarkTitle.textContent = bookmark.problemName;
...
function onPlay(event){
    window.open(event.target.parentNode.parentNode.getAttribute("url"));
}

function onDelete(event){
    const bookmarkItem = event.target.parentNode.parentNode;
    if(bookmarkItem.parentNode.children.length === 1){
        viewBookmarks([]);
    }
